...
const Section: React.FC<{ title: string; children: React.ReactNode, viewMore?: boolean, onViewMore?: () => void }> = ({ title, children, viewMore = true, onViewMore }) => (
    <section className="mb-16 animate-fade-in relative z-10 font-fredoka">
      <div className="flex justify-between items-end mb-6 px-2 sm:px-4">
        <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl md:text-3xl font-normal text-white tracking-tighter leading-none uppercase drop-shadow-md">
                {title}
            </h2>
        </div>
        {viewMore && (
            <button 
                onClick={onViewMore} 
                className="group flex items-center gap-2 px-4 py-1.5 rounded-full bg-gray-900 border border-gray-700 hover:border-purple-500/50 transition-all duration-300 hover:shadow-[0_0_15px_rgba(147,51,234,0.3)] font-sans"
            >
...
const Home: React.FC<HomeProps> = ({ games }) => {
  ...
  const renderHero = () => {
    ...
    return (
        <div className="relative h-[400px] w-full rounded-3xl overflow-hidden mb-16 shadow-[0_20px_50px_rgba(0,0,0,0.5)] group border border-white/5 mx-auto max-w-[1400px] font-fredoka">
            <Image 
            ...
            <div className="absolute inset-0 p-8 md:p-16 flex flex-col justify-center max-w-4xl">
                <div className="space-y-6 animate-fade-in-right">
                    <h2 
                        className="text-5xl md:text-7xl font-normal text-white leading-none tracking-tighter drop-shadow-2xl uppercase" 
                        dangerouslySetInnerHTML={{ __html: settings.hero_title }} 
                    />
                    <p className="text-xl text-gray-300 font-medium drop-shadow-md max-w-2xl border-l-4 border-purple-500 pl-6 py-1 font-sans">
                        {settings.hero_subtitle}
                    </p>
                    <div className="pt-6">
                        <button 
                            onClick={() => router.push(settings.hero_button_url)} 
                            className="group relative bg-white text-[#0d0d0d] font-normal py-4 px-10 text-lg rounded-xl overflow-hidden transition-all hover:scale-105 hover:shadow-[0_0_30px_rgba(255,255,255,0.4)] uppercase tracking-wide skew-x-[-5deg]"
                        >
                            <span className="relative z-10 flex items-center gap-3 skew-x-[5deg]">
                                {settings.hero_button_text}
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 transform group-hover:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                            </span>
  ...
  const renderPromo = () => {
      if (!isLoading && settings.promo_enabled) {
          return (
              <div className="relative overflow-hidden rounded-2xl p-0.5 bg-gradient-to-r from-yellow-600 via-orange-500 to-red-600 mb-16 shadow-[0_0_30px_rgba(234,179,8,0.25)] max-w-[1400px] mx-auto group font-fredoka">
                  <div className="relative bg-[#121212] rounded-[0.9rem] p-6 flex flex-col md:flex-row items-center justify-between gap-6 overflow-hidden">
                      <div className="flex items-center space-x-5 relative z-10">
                          ...
                          <div>
                              <h3 className="text-xl md:text-2xl font-normal text-white mb-1 tracking-tight uppercase">{settings.promo_text}</h3>
                              <p className="text-gray-400 text-sm font-medium font-sans">Limited time event. Join now!</p>
                          </div>
                      </div>
                      <button 
                        onClick={() => router.push(settings.promo_button_url)} 
                        className="relative z-10 bg-white hover:bg-gray-100 text-black font-normal py-3 px-8 rounded-lg transition-all hover:scale-105 hover:shadow-lg whitespace-nowrap text-sm uppercase tracking-wide"
                      >
