...
const GameDetailPage: React.FC<GameDetailPageProps> = ({ game, similarGames }) => {
    ...
    // Map view_count to "Hours played" display
    ...
    return (
        <>
            ...
            <div className="fixed inset-0 bg-black text-white font-sans overflow-hidden select-none">
                ...
                {/* 1. IMMERSIVE BACKGROUND (Mapped to background_url) */}
                <div className="absolute inset-0 z-0">
                    <Image 
                        ...
                        unoptimized
                    />
                    {/* Cinematic Gradients to match the image */}
                    <div className="absolute inset-0 bg-gradient-to-r from-black/70 via-black/20 to-transparent" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-black/30" />
                </div>

                {/* 2. PS5 TOP BAR (Games, Media, Search, Settings, Clock) */}
                ...
                {/* 3. GAME ICON STRIP (Similar Games) */}
                <div className="relative z-40 px-12 mt-8 flex items-end gap-5">
                    {/* Active Game Icon */}
                    <div className="flex-shrink-0 w-32 h-32 rounded-[2.2rem] overflow-hidden border-[5px] border-white shadow-[0_0_60px_rgba(255,255,255,0.4)] transition-all transform scale-110 z-10 relative bg-gray-900">
                        <Image src={game.imageUrl} alt="" fill className="object-cover" unoptimized />
                    </div>
                    {/* Similar Games as dimmed icons */}
                    {similarGames.slice(0, 9).map(sg => (
                ...
                <div className="absolute left-12 bottom-36 right-12 z-20">
                    <div className="flex justify-between items-end">
                        ...
                        {/* Hours Played Stat (Mapped to view_count) */}
                ...
                <div className="absolute left-12 bottom-10 z-20 flex gap-8 w-full">
                    ...
                    <div className="w-[520px] bg-white/10 backdrop-blur-3xl rounded-[2.8rem] p-8 border border-white/10 flex items-center gap-8 hover:bg-white/15 transition-all shadow-2xl">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-white/40 text-sm font-black uppercase tracking-widest">{game.rating || 98}% Completion Rate</p>
                        </div>
                    </div>

                    {/* Friends Card (Mapped to downloadsCount) */}
                    <div className="w-[520px] bg-white/10 backdrop-blur-3xl rounded-[2.8rem] p-8 border border-white/10 flex items-center gap-8 hover:bg-white/15 transition-all shadow-2xl">
                        ...
                        <div className="flex-1">
                            <div className="flex -space-x-3.5 mb-2.5">
                                {[1,2,3,4,5].map(i => <div key={i} className="w-9 h-9 rounded-full border-[3px] border-[#111] bg-gray-700 overflow-hidden relative shadow-lg"><Image src={`https://i.pravatar.cc/60?u=${i+30}`} alt="" fill /></div>)}
                            </div>
                            <h4 className="text-2xl font-black uppercase tracking-tight mb-0.5">Friends Who Play</h4>
                            <p className="text-white/40 text-sm font-black uppercase tracking-widest">{(game.downloadsCount || 1500).toLocaleString()} Active Players</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <style jsx global>{`
                ...
                /* Custom styles to hide standard layout stuff if needed */
