...
interface GameCardProps {
  ...
  isFocused?: boolean; // New prop for XMB-style focus
}

const GameCard: React.FC<GameCardProps> = ({ game, variant = 'default', isFocused = false }) => {
  ...
  return (
    <Link 
      ...
      className={`group flex flex-col w-full h-full bg-[#0e0e12] p-3 rounded-[32px] transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] 
                 ${active ? 'bg-[#13131a] scale-105 z-20 border-purple-500/40 shadow-[0_0_40px_rgba(147,51,234,0.15)]' : 'opacity-60 grayscale-[0.4] scale-95 border-white/5'} 
                 border hover:grayscale-0 hover:opacity-100`}
    ...
    >
      ...
      <div className="flex justify-between items-start mb-5 px-1 gap-2">
         <div className="flex flex-col gap-2 min-w-0 flex-1">
            <h3 className={`font-bold text-lg leading-none truncate transition-colors ${active ? 'text-purple-400' : 'text-white'}`}>
                {game.title}
            </h3>
         ...
         <div className={`relative w-11 h-11 rounded-2xl overflow-hidden shrink-0 border-2 transition-all duration-500 ${active ? 'border-purple-500 scale-110 rotate-3' : 'border-[#1c1c24]'} shadow-sm bg-gray-800`}>
             <Image 
                src={profileImageSrc} 
                alt="" 
                fill 
      ...
      {/* 3. Bottom HUD */}
      <div className={`flex items-center gap-3 mt-auto transition-all duration-500 ${active ? 'opacity-100 translate-y-0' : 'opacity-40 -translate-y-1'}`}>
