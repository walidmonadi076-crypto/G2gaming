...
const StoreItemCard: React.FC<StoreItemCardProps> = ({ product }) => {
  return (
    <Link href={`/shop/${product.slug}`} className="block group">
      <div className="bg-gray-800 rounded-xl overflow-hidden aspect-square relative mb-4">
        <Image 
      ...
      <div className="space-y-1">
        ...
        <h3 className="text-lg font-medium text-white truncate group-hover:text-purple-400 transition-colors">{product.name}</h3>
        <div className="flex items-center justify-between pt-2">
            ...
            <div className="opacity-0 group-hover:opacity-100 transition-opacity transform translate-x-2 group-hover:translate-x-0 duration-300">
                <span className="text-sm font-bold text-purple-400 uppercase tracking-wider flex items-center gap-1">
                    Buy Now 
